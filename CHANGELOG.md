...
- **WS 打开超时兜底（10s）**：连接卡在 CONNECTING（网络黑洞/代理挂起、半开受限网络）时自动主动关闭，转入现有指数退避重连，避免连接状态永久僵死；`connectWs` 同时清理残留的 CONNECTING 旧连接
- **手机端提示音静默修复（iOS/部分安卓）**：根因是 AudioContext 原在收到 WS 消息的**非用户手势上下文**中惰性创建，而 Web Audio 要求在手势内启动才能解锁——context 永久 suspended、`currentTime` 冻结 → 播放静默无声。现改为：页面所有用户手势事件（click/touchstart/keydown/pointerdown，passive）统一绑定 `unlockAudio()` 在手势内完成创建+`resume()` 解锁；播放前若仍挂起则 `await ctx.resume()` 后再调度音符；上下线/新消息两处音效共用同一解锁逻辑。效果：开启声音提醒后，首次任意点按页面即完成音频解锁，后续收到消息/上下线提示正常发声
